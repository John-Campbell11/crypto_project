        ...
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mmapped_file:
            ...
            while file_pos < file_size - 8:
                ...
                for _ in range(tx_count):
                    ...
                    txid, version, inputs_list, outputs_list, locktime, is_coinbase, segwit = tx
                    txs_batch.append((txid, block_hash, is_coinbase))

                    for vout, (value, script_pubkey) in enumerate(outputs_list):
                        script_type = get_script_type_fast(script_pubkey)
                        outputs_batch.append((txid, vout, value, script_pubkey, script_type))

                    for vin, (prev_hash, prev_index, sequence) in enumerate(inputs_list):
...
# --- Main ---
def main():
    # Load completed files from progress log
    ...
    dat_files = sorted(glob.glob(os.path.join(DAT_DIR, "blk*.dat")))
    dat_files = [f for f in dat_files if f not in completed_files]
    ...
    for fpath in dat_files:
        ...
        # Log completion for resumability
